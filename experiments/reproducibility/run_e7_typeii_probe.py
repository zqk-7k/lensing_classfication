...
def parse_args():
    ...
    parser.add_argument("--manifest", default=str(ROOT / "experiments/reproducibility/manifests/e7_typeii/e7_source_manifest_seed20260711.csv.gz"))
    parser.add_argument("--output-dir", default=str(ROOT / "results/diagnostics/type_ii"))
...
def main():
    ...
    checkpoint = ROOT / f"artifacts/training/pi_resnet_{lower}_noisy_seed42/best.pt"
    device = torch.device(args.device)
    model = PIResNet(in_channels=1, d_model=256, width_scale=4.0,
                                            use_snake=False, use_se=True, use_pairwise_fusion=True).to(device)
    ...
    core = json.load(open(ROOT / "results/core/core_results.json"))
