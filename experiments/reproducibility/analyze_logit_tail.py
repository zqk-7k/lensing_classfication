...
for lens in ("sis", "pm"):
    ...
    for model, (score, logit) in definitions.items():
        ...
        for fpp in (1e-3, 1e-4):
            score_t = float(np.quantile(cal.loc[cal.label == 0, score], 1-fpp, interpolation="higher"))
            logit_t = float(np.quantile(cal.loc[cal.label == 0, logit], 1-fpp, interpolation="higher"))
