#!/usr/bin/env python3
"""Reproducible CQT--DeiT baseline training on the 0222 catalog."""
...
from dataset import CQTPairDataset, get_evaluation_transforms, get_training_transforms  # noqa: E402
...
def parse_args():
    ...
    parser.add_argument("--image-root", default=os.environ.get("GW_DATA_ROOT", str(ROOT / "data")))
    ...
    parser.add_argument(
        "--pretrained-weights",
        default=str(ROOT / "artifacts" / "pretrained" /
...
def main():
    ...
    train_ds = CQTPairDataset(train_paths, train_labels, transform=get_training_transforms())
    val_ds = CQTPairDataset(val_paths, val_labels, transform=get_evaluation_transforms())
    ...
    metadata = vars(args) | {
        ...
        "selection_metric": "validation_auc", "implementation": "CQT-DeiT (SEMD-inspired)",
    ...
    train_eval_ds = CQTPairDataset(train_paths, train_labels, transform=get_evaluation_transforms())
