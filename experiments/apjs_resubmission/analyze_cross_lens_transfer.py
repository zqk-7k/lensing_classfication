...
PRED = ROOT / "runs/apjs_resubmission_final_v1/predictions_0228"
OUT = ROOT / "runs/apjs_resubmission_final_v1/cross_lens_0228"
...
for target, source in (("sis", "pm"), ("pm", "sis")):
    ...
    for model, prefix, score in (("pi_resnet", "pi", "pi_score"), ("cqt_deit", "cqt_deit", "cqt_deit_score")):
        ...
        in_threshold = float(np.quantile(within.loc[cal_mask & (within.label == 0), score], .999, method="higher"))
        cross_threshold = float(np.quantile(cross.loc[cal_mask & (cross.label == 0), score], .999, method="higher"))
