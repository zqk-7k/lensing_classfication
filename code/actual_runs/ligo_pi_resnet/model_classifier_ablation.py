...
class BinaryPeriodicResNet1D_Ablation(nn.Module):
    # 【新增】in_channels 参数，默认设为 1
    def __init__(self, in_channels=1, d_model=256, width_scale=4.0, use_snake=True, use_se=True, use_physics_fusion=True):
        super().__init__()
        ...
        self.stem = nn.Sequential(
            nn.Conv1d(in_channels, base, kernel_size=15, stride=2, padding=7), # 【修改】使用传入的 in_channels
            nn.BatchNorm1d(base),
            get_act(base),
    ...
    def forward(self, x):
        ...
        # 【核心修改】动态切分通道，完美兼容 ET (拼接后 2 通道) 和 LIGO (拼接后 4 通道)
        ch = x.shape[1] // 2
        x1 = x[:, :ch, :]  # 分支 1 信号 [B, ch, L]
        x2 = x[:, ch:, :]  # 分支 2 信号 [B, ch, L]
        # === 添加这句测试 ===
        #print(f"Branch 1 Shape: {x1.shape}, Branch 2 Shape: {x2.shape}")
