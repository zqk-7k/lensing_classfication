...
def plot_sis_error_hist():
    ...
    # 5. 准备柱状图数据 (仅分析 Lensed 信号)
    snrs, probs, labels = np.array(snrs), np.array(probs), np.array(labels)
    preds = (probs >= 0.5).astype(int)

    lensed_mask = (labels == 1)
    tp_snrs = snrs[lensed_mask & (preds == 1)] # 正确检出的 SNR
    fn_snrs = snrs[lensed_mask & (preds == 0)] # 漏掉的信号 SNR

    # 6. 绘图
    plt.figure(figsize=(9, 7))
    bins = np.linspace(0, 20, 21) # 0到20，每格为1
    ...
    print("✅ SIS 误差分布图已生成: pic/fig11_sis_hist_new.png")
