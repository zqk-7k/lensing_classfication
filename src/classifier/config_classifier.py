...
# ========================== 核心开关 ==========================
# 1. 任务模式
DATA_MODE = "pure"       # "pure" (纯信号) 或 "noisy" (信号+噪声)
MODEL_TYPE = "SIS"        # "SIS" 或 "PM"
...
if MODEL_TYPE == "SIS":
    SOURCE_DIR = os.path.join(DATA_ROOT, "SIS_data")
    FILE_PREFIX = "SIS"
elif MODEL_TYPE == "PM":
    SOURCE_DIR = os.path.join(DATA_ROOT, "PM_data")
    FILE_PREFIX = "PM"
else:
...
UNL_PATHS = [os.path.join(DATA_ROOT, "Unlensed_data", UNL_FILENAME)]

# 输出目录
OUT_DIR = os.path.join(PROJECT_ROOT, "runs", f"classifier_{MODEL_TYPE}_{DATA_MODE}_v1")
