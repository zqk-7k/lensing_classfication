...
def main():
    ...
    # ================= [修复核心] 动态分配路径并强制创建目录 =================
    if args.dataset == 'SIS':
        cfg.SOURCE_DIR = os.path.join(cfg.DATA_ROOT, "SIS_data")
        cfg.OUT_DIR = os.path.join(cfg.PROJECT_ROOT, "runs", "classifier_SIS_noisy_v1")
    else:
        cfg.SOURCE_DIR = os.path.join(cfg.DATA_ROOT, "PM_data")
        cfg.OUT_DIR = os.path.join(cfg.PROJECT_ROOT, "runs", "classifier_PM_noisy_v1")
    os.makedirs(cfg.OUT_DIR, exist_ok=True)  # 就是这句救命的代码！
    os.makedirs(os.path.join(cfg.PROJECT_ROOT, "runs", "logs"), exist_ok=True)
    # =========================================================================
    ...
    unl_path = os.path.join(cfg.DATA_ROOT, "Unlensed_data", "unlensed_data_strain.npy")
    ...
    # ================= [核心修改] 动态设置输入通道 =================
    # SIS (LIGO) 是双探测器数据 (H1, L1)，所以输入通道是 2
    # PM (ET) 是单探测器数据，所以输入通道是 1
    # 注意：这里需要确保你在 model_classifier_ablation.py 中也添加了 in_channels 参数
    in_ch = 2 if args.dataset == 'SIS' else 1
    # =============================================================

    model = BinaryPeriodicResNet1D_Ablation(
        in_channels=in_ch,   # <--- 传入这个动态参数
        d_model=256, 
        width_scale=4.0,
